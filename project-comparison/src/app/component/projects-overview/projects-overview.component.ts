...
import { IPolicyCategory, IProject } from 'src/app/structures';
...
export class ProjectsOverviewComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        forkJoin([this.apiService.getPolicyCategories(), this.apiService.getMethodologies()]).subscribe(([policyCategories, methodologies]) => {
            this.loading = false;
    ...
    compareExecute() {
        ...
        this.router.navigate(['/projects'], {queryParams: {projectIds: projectIds.join(',')}});
    }

    searchByFilter() {
        this.loadingProjects = true;
        this.apiService.getFilteredProjects(this.selectedCategoryIds, this.selectedMethodologyIds).subscribe((result: any) => {
            this.allProjects = result;
            this.filterByInput();

            const projectIds = JSON.parse(localStorage.getItem('active-projects')) as string[];
            if (projectIds) {
    ...
    private trackFilterChanges() {
        this.filterFormGroup.valueChanges.subscribe(() => {
            ...
            this.apiService.getMethodologies(categoryIds).subscribe(result => {
